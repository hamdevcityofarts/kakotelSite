...
const Features = ({ id, header, items, columns = 3, hasBackground = false }: FeaturesProps) => (
  <WidgetWrapper id={id ? id : ''} hasBackground={hasBackground} containerClass="scroll-mt-16 max-w-6xl">
    {header && <Headline header={header} titleClass="text-xs md:text-3xl" />}
    <ItemGrid
      id={id}
      items={items}
      columns={columns}
      ...
      iconClass="h-12 w-12 flex items-center justify-center rounded-md text-white bg-[#f8ca01] p-2 md:p-3 mt-1.5 mb-4 mr-4 rtl:ml-4 rtl:mr-0"
      titleClass="mb-3 text-sm font-bold text-[#f8ca01]"
